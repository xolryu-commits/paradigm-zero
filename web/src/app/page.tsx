...
function GameContent({ onReboot }) {
  ...
  const [warningEditIndex, setWarningEditIndex] = useState(null); // 수정 시 인덱스 저장 (null이면 추가)
  ...
  const getNodeStatus = (nodeId) => {
    ...
    // 중요 수정: 현재 위치가 '점령된 상태'여야만 인접 지역이 'Attackable'로 표시됨
    // 즉, 점령하지 않고 이동만 한 상태라면 다음 경로는 잠김(Locked) 상태 유지
            ...
            <div className="flex border-b border-slate-800">
               <button 
                 ...
                 className={`flex-1 py-3 text-xs font-bold flex items-center justify-center gap-2 transition-colors
                   ${adminTab === 'map' ? 'bg-slate-800 text-red-400 border-b-2 border-red-500' : 'text-slate-500 hover:text-slate-300'}`}
               >
                 <Crosshair size={14} /> TACTICAL CONTROL
               </button>
               <button 
                 onClick={() => setAdminTab('database')}
                 className={`flex-1 py-3 text-xs font-bold flex items-center justify-center gap-2 transition-colors
                   ${adminTab === 'database' ? 'bg-slate-800 text-red-400 border-b-2 border-red-500' : 'text-slate-500 hover:text-slate-300'}`}
               >
                 <Database size={14} /> NODE DATABASE
               </button>
            </div>
          )}

          {/* Tab Content: Database Editor */}
          {isAdmin && adminTab === 'database' ? (
             <div className="flex-1 overflow-y-auto p-4 bg-slate-925">
               <h3 className="text-slate-500 text-xs font-bold mb-4 flex items-center gap-2">
                 <Database size={14} /> FULL NODE LIST ACCESS
               </h3>
                   ...
                   <div key={node.id} className="bg-slate-900 border border-slate-700 p-4 rounded hover:border-slate-600 transition-colors">
                     <div className="flex items-center gap-2 mb-3">
                       <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold
                          ${node.type === 'deadend' ? 'bg-orange-900 text-orange-400' : 'bg-slate-800 text-slate-400'}
                       `}>
                         {node.id}
                       </div>
                       <input 
                         ...
                         className="flex-1 bg-transparent border-b border-slate-700 focus:border-red-500 outline-none text-white font-bold"
                       />
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                       <div>
                          ...
                          <textarea 
                            value={node.desc}
                            onChange={(e) => handleUpdateNode(node.id, 'desc', e.target.value)}
                            className="w-full bg-slate-950 border border-slate-800 rounded p-2 text-xs text-slate-300 focus:border-red-500 outline-none resize-none h-16"
                          />
                       </div>
                       <div>
                          <label className="text-[10px] text-orange-500 uppercase font-bold block mb-1">
                            Custom Warnings
                            <button 
                              onClick={() => openWarningModal(node.id)}
                              className="ml-2 px-2 py-0.5 bg-blue-900 text-blue-300 text-[10px] rounded hover:bg-blue-800 inline-flex items-center gap-1"
                            >
                              <Plus size={10} /> Add
                            </button>
                          </label>
                          <div className="w-full bg-slate-950 border border-slate-800 rounded p-2 min-h-[4rem] space-y-1">
                            {(!node.customWarnings || node.customWarnings.length === 0) && (
                              <span className="text-xs text-slate-600 italic">No warnings set</span>
                            )}
                            {node.customWarnings?.map((warning, idx) => (
                              <div key={idx} className="flex items-center justify-between bg-red-900/20 px-2 py-1 rounded border border-red-900/30">
                                ...
                                <div className="flex items-center gap-1">
                                  <button onClick={() => openWarningModal(node.id, idx, warning)} className="text-slate-500 hover:text-white">
                                    <Edit3 size={12} />
                                  </button>
                                  <button onClick={() => handleRemoveWarning(node.id, idx)} className="text-slate-500 hover:text-red-400">
                                      <Trash2 size={14} />
                                  </button>
                                </div>
                              </div>
                          ...
                          {node.type === 'deadend' && (
                             <div className="mt-2">
                                <label className="text-[10px] text-orange-500 uppercase font-bold block mb-1">Dead End Reveal Text</label>
                                <textarea 
                                  value={node.revealDesc}
                                  onChange={(e) => handleUpdateNode(node.id, 'revealDesc', e.target.value)}
                                  className="w-full bg-slate-950 border border-orange-900/30 rounded p-2 text-xs text-orange-300 focus:border-orange-500 outline-none resize-none h-10"
                                />
                             </div>
                          )}
                       </div>
                ...
                {isAdmin && (
                  <div className="mb-6 bg-slate-950/50 p-3 rounded border border-slate-700 flex items-center justify-between">
                    <span className="text-xs font-bold text-slate-400 flex items-center gap-2">
                      <Calendar size={14} /> OPERATION DAY
                    </span>
                    ...
                    <div className="flex flex-col gap-2">
                      {isAdmin ? (
                        <input 
                          ...
                          className="bg-slate-800 border border-slate-600 rounded p-2 text-lg font-bold text-white focus:border-red-500 focus:outline-none"
                        />
                      ) : (
                        <h3 className="text-2xl font-bold text-white">{selectedNode.label}</h3>
                      )}

                      <div className={`px-2 py-1 rounded text-xs font-bold uppercase self-start
                        ${getNodeStatus(selectedNode.id) === 'captured' ? 'bg-emerald-900 text-emerald-400' : 
                          getNodeStatus(selectedNode.id) === 'attackable' ? 'bg-yellow-900 text-yellow-400' :
                          getNodeStatus(selectedNode.id) === 'current' ? 'bg-blue-900 text-blue-400' : 'bg-red-900 text-red-400'
                        }`}>
                      ...
                      <div className="space-y-2">
                        <div>
                          ...
                          <textarea 
                            ...
                            className="w-full h-20 bg-slate-800 border border-slate-600 rounded p-2 text-sm text-slate-300 focus:border-red-500 focus:outline-none resize-none"
                          />
                        </div>
                        
                        {/* Custom Warning Management in Map Panel */}
                        <div>
                            <div className="flex items-center justify-between mb-1">
                                <label className="text-xs text-red-500 font-bold">CUSTOM WARNINGS</label>
                                <button 
                                  onClick={() => openWarningModal(selectedNode.id)}
                                  className="text-[10px] bg-slate-800 px-2 py-1 rounded hover:bg-slate-700 text-white flex items-center gap-1"
                                >
                            ...
                            <div className="space-y-2">
                                {selectedNode.customWarnings?.map((warning, idx) => (
                                    <div key={idx} className="flex items-center gap-2 bg-slate-800 p-2 rounded border border-slate-700">
                                        ...
                                        <div className="flex items-center gap-1">
                                          <button onClick={() => openWarningModal(selectedNode.id, idx, warning)} className="text-slate-500 hover:text-white">
                                            <Edit3 size={12} />
                                          </button>
                                          <button onClick={() => handleRemoveWarning(selectedNode.id, idx)} className="text-slate-500 hover:text-red-400">
                                              <Trash2 size={14} />
                                          </button>
                                        </div>
                                    </div>
                                ))}
                                {(!selectedNode.customWarnings || selectedNode.customWarnings.length === 0) && (
                                    <div className="text-xs text-slate-600 italic p-2 text-center border border-slate-800 border-dashed rounded">
                                        No warnings active
                                    </div>
                                )}
                            </div>
                        ...
                        {selectedNode.type === 'deadend' && (
                          <div className="mt-2">
                            ...
                            <textarea 
                              value={selectedNode.revealDesc}
                              onChange={(e) => handleUpdateNode(selectedNode.id, 'revealDesc', e.target.value)}
                              className="w-full h-20 bg-slate-800 border border-orange-900/50 rounded p-2 text-sm text-orange-300 focus:border-orange-500 focus:outline-none resize-none"
                            />
                          </div>
                        )}
                        <div className="text-xs text-slate-500 italic mt-2 text-right">
                           * 더 많은 수정은 상단 'NODE DATABASE' 탭에서 가능합니다.
                        </div>
                      </div>
                    ) : (
                      <p className="text-slate-300 leading-relaxed text-sm">
                        {selectedNode.type === 'deadend' && capturedNodes.includes(selectedNode.id) && selectedNode.revealDesc
                          ? selectedNode.revealDesc 
                          : selectedNode.desc}
                      </p>
                    )}

                    {/* Display Custom Warnings (For Users & Admin) */}
                       ...
                       {isAdmin ? (
                         <>
                           <div className="grid grid-cols-2 gap-2">
                              <button 
                                  onClick={toggleNodeCapturedStatus}
                                  className={`py-3 font-bold rounded flex flex-col items-center justify-center gap-1 transition-colors border text-xs
                                    ${capturedNodes.includes(selectedNode.id) 
                                      ? 'bg-slate-800 border-slate-600 text-slate-400 hover:bg-slate-700' 
                                      : 'bg-emerald-900/50 border-emerald-500 text-emerald-400 hover:bg-emerald-900'
                                    }`}>
                              ...
                              <button 
                                  onClick={adminMoveUnit}
                                  disabled={selectedNode.id === currentLocation}
                                  className={`py-3 font-bold rounded flex flex-col items-center justify-center gap-1 transition-colors border text-xs
                                    ${selectedNode.id === currentLocation
                                      ? 'bg-slate-800 border-slate-700 text-slate-600 cursor-not-allowed'
                                      : 'bg-blue-900/50 border-blue-500 text-blue-400 hover:bg-blue-900' 
                                    }`}>
                                  <Navigation size={16} />
                                  부대 이동
                              </button>
                         ...
                         <>
                            {getNodeStatus(selectedNode.id) === 'attackable' && (
                              <div className="w-full py-3 bg-slate-900 border border-yellow-700/50 text-yellow-600 rounded flex items-center justify-center gap-2 cursor-default opacity-70">
                                <Lock size={20} />
                                작전 승인 대기 (PENDING)
                              </div>
                            )}
                            {getNodeStatus(selectedNode.id) === 'captured' && selectedNode.id !== currentLocation && (
                              <div className="w-full py-3 bg-slate-900 border border-emerald-700/50 text-emerald-600 rounded flex items-center justify-center gap-2 cursor-default opacity-70">
                                 <Shield size={20} />
                                 확보된 지역 (SECURED)
                              </div>
                            )}
                            {getNodeStatus(selectedNode.id) === 'current' && (
                               <div className="w-full py-3 bg-blue-900/20 border border-blue-500/30 text-blue-400 rounded flex items-center justify-center gap-2 cursor-default">
                                 <MapPin size={20} />
                                 현재 주둔지 (CURRENT LOCATION)
                               </div>
                            )}
                            {getNodeStatus(selectedNode.id) === 'locked' && (
                                <div className="w-full py-3 bg-slate-900 text-slate-600 rounded flex items-center justify-center gap-2 cursor-default border border-slate-800">
                                  <Lock size={16} />
                                  경로 미확보 (LOCKED)
                                </div>
                            )}
